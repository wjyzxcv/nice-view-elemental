...
static void
connectivity_state_update_callback(struct connectivity_state state) {
  states.connectivity = state;

  render_connectivity();
}

static struct connectivity_state
get_connectivity_state(const zmk_event_t *event) {
  const struct zmk_endpoint_instance selected_endpoint =
...
ZMK_DISPLAY_WIDGET_LISTENER(
    widget_connectivity_state_update, struct connectivity_state,
    // Called after `get_connectivity_state` with the value it returned.
    connectivity_state_update_callback, get_connectivity_state)

// Subscribe the `widget_connectivity_state_update` listener to the
// `zmk_endpoint_changed` event dispatched by ZMK.
ZMK_SUBSCRIPTION(widget_connectivity_state_update, zmk_endpoint_changed);

// Subscribe the `widget_connectivity_state_update` listener to the
// `zmk_usb_conn_state_changed` event dispatched by ZMK.
ZMK_SUBSCRIPTION(widget_connectivity_state_update,
                 // Triggered when connected or disconnected from USB.
                 zmk_usb_conn_state_changed);
...
static struct layer_state get_layer_state(const zmk_event_t *event) {
  ...
  struct layer_state state = {.index = index, .name = name};
...
ZMK_DISPLAY_WIDGET_LISTENER(
    widget_layer_state_update, struct layer_state,
    // Called after `get_layer_state` with the value it returned.
    layer_state_update_callback, get_layer_state)

// Subscribe the `widget_layer_state_update` listener to the
// `zmk_battery_state_changed` event dispatched by ZMK.
ZMK_SUBSCRIPTION(widget_layer_state_update,
...
// Create a listener named `widget_modifiers_state_update`. This name is then
// used to create a subscription.
ZMK_DISPLAY_WIDGET_LISTENER(
    widget_modifiers_state_update, struct modifiers_state,
    // Called after `get_modifiers_state` with the value it returned.
    modifiers_state_update_callback, get_modifiers_state)

// Subscribe the `widget_modifiers_state_update` listener to the
// `zmk_keycode_state_changed` event dispatched by ZMK.
ZMK_SUBSCRIPTION(widget_modifiers_state_update, zmk_keycode_state_changed);

#if IS_ENABLED(CONFIG_NICE_VIEW_ELEMENTAL_ANIMATION)
// Pause/resume animation based on keyboard activity
...
ZMK_LISTENER(activity_update, activity_update_callback);
ZMK_SUBSCRIPTION(activity_update, zmk_activity_state_changed);
#endif
